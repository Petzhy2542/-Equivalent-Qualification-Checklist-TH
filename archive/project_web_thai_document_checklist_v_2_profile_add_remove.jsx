// [PROJECT Web] – Web App + UX/UI Design for Canvas
// Thai Document Checklist v3.1.1 (Multi-Applicant + Rename/Duplicate + Delete fix)
// -----------------------------------------------------------------------------
// What's new in v3.1.1
// - แก้ปุ่ม "ลบรายชื่อนี้" ให้ทำงานเสถียร: เลือกแท็บถัดไปอัตโนมัติ, กันลบจนเหลือ 0 รายชื่อ
// - เพิ่มปุ่มลบในแถบแท็บรายชื่อ พร้อมปิดการกดเมื่อเหลือเพียง 1 รายชื่อ และมียืนยันก่อนลบ
// - แนะนำการใช้งาน: ถ้าเหลือรายชื่อเดียว ให้ใช้ปุ่ม "รีเซ็ต (รายชื่อนี้)" แทนการลบ
// -----------------------------------------------------------------------------

import React, { useEffect, useMemo, useRef, useState } from "react";

// Utility Types
/** @typedef {"required" | "optional"} Requirement */
/** @typedef {"todo" | "done"} Status */
...
// Helpers
const cls = (...tokens) => tokens.filter(Boolean).join(" ");
const casesKey = "doc-cases-v1";
const activeKey = "doc-activeCase-v1";
const legacyKeys = {
  sections: "doc-checklist-v2",
  profile: "doc-profile-v1",
  applicant: "doc-applicant-v1",
};

const makeSections = () =>
  DEFAULT_DATA.map((sec) => ({
    ...sec,
    open: true,
...
// Initialize cases (with legacy migration v2 -> v3)
const INITIAL_CASES = (() => {
  try {
    const raw = localStorage.getItem(casesKey);
    if (raw) return JSON.parse(raw);
  } catch {}
  // Try migrate from v2 (single profile)
  try {
...
function isItemApplicable(sectionId, itemId, profile) {
  // Thai Visa item only for Non-Thai
...
export default function App() {
  ...
  // Search & filter (current case only)
  const [query, setQuery] = useState("");
  const [filter, setFilter] = useState("all"); // all | required | optional | done | todo
  ...
  // Update helpers for current case
  ...
  function toggleItem(sectionId, itemId) {
    updateCurrent((c) => ({
      ...
      sections: c.sections.map((s) =>
        s.id !== sectionId
          ? s
          : { ...s, items: s.items.map((i) => (i.id === itemId ? { ...i, status: i.status === "done" ? "todo" : "done" } : i)) }
      ),
    }));
  }
  ...
  function addItem(sectionId, label, req) {
    ...
    updateCurrent((c) => ({
      ...
      sections: c.sections.map((s) =>
        s.id !== sectionId
          ? s
          : { ...s, items: [...s.items, { id: newId, label: label.trim(), req: req === "required" ? "required" : "optional", status: "todo", file: "", note: "", custom: true }] }
      ),
    }));
  }

  // Case ops (rename/duplicate/delete)
  function renameCase(caseId, newLabel) {
    setCases((prev) => prev.map((c) => (c.id === caseId ? { ...c, label: newLabel || c.label } : c)));
  }
  ...
  function removeCase(caseId, labelHint = "") {
    setCases((prev) => {
      if (prev.length <= 1) {
        alert("ต้องมีอย่างน้อย 1 รายชื่อ — หากต้องการเริ่มใหม่ ใช้ปุ่ม ‘รีเซ็ต (รายชื่อนี้)’ แทน");
        return prev;
      }
      ...
      // กำหนด active ใหม่: ถ้าลบแท็บปัจจุบัน ให้เลือกแท็บซ้ายมือ (หรือแท็บแรกเมื่อไม่มีซ้าย)
      if (activeId === caseId) {
  ...
  function setAllOpen(open) {
    updateCurrent((c) => ({ ...c, sections: c.sections.map((s) => ({ ...s, open })) }));
  }

  // Reset current case to template
  function resetCurrentCase() {
    if (!confirm("รีเซ็ตสถานะทั้งหมดของรายชื่อนี้และโหลดรายการมาตรฐาน?")) return;
    updateCurrent((c) => ({ ...c, sections: makeSections() }));
  }
  ...
  function handleExport() {
    ...
    a.href = url;
    a.download = `document-checklist-${new Date().toISOString().slice(0, 10)}.json`;
    a.click();
    ...
    reader.onload = () => {
      try {
        ...
        if (data && Array.isArray(data.cases)) {
          if (!data.cases.length) throw new Error("empty cases");
          setCases(data.cases);
          setActiveId(data.activeId || data.cases[0].id);
          alert("นำเข้าเรียบร้อย (หลายรายชื่อ)");
          return;
        }
        // v2 fallback (single profile)
        if (data && Array.isArray(data.sections)) {
          const one = makeCase(data.applicant?.fullName || "ผู้ยื่น 1", data.applicant || {}, data.profile || {});
          one.sections = data.sections.map((sec) => ({
            ...sec,
            open: typeof sec.open === "boolean" ? sec.open : true,
            items: (sec.items || []).map((i) => ({
              id: i.id,
          ...
          setCases([one]);
          setActiveId(one.id);
          alert("นำเข้าเรียบร้อย (ไฟล์เดิม v2)");
          return;
        }
        throw new Error("unsupported format");
      } catch (e) {
        alert("ไม่สามารถนำเข้าไฟล์นี้ได้");
      }
    };
  ...
  // Keyboard shortcuts (search/save toast)
  useEffect(() => {
    const handler = (e) => {
  ...
  function showSavedToast() {
    ...
    setTimeout(() => btn?.classList.add("opacity-0"), 1000);
  }
  ...
  return (
    <div className="min-h-screen bg-gradient-to-b from-neutral-50 to-neutral-100 text-neutral-900">
      ...
      <main className="mx-auto max-w-5xl px-4 pb-24">
        ...
        {/* Applicant Info & Profile for current case */}
        <ApplicantCard
        ...
        <SectionList
          ...
          onToggleOpen={(id) =>
            updateCurrent((c) => ({ ...c, sections: c.sections.map((s) => (s.id === id ? { ...s, open: !s.open } : s)) }))
          }
      ...
      {importOpen && (
        <ImportModal onClose={() => setImportOpen(false)} onPickFile={() => fileInputRef.current?.click()} />
      )}
      {addOpen && (
        <AddApplicantModal
      ...
      <input
        ...
        className="hidden"
        onChange={(e) => handleImportFromFile(e.target.files?.[0])}
      />
      ...
      <div
        id="save-toast"
        aria-live="polite"
        className="fixed bottom-4 right-4 rounded-xl bg-neutral-900/90 px-3 py-2 text-sm text-white shadow-xl transition-opacity duration-300 opacity-0"
      >
        บันทึกแล้ว ✓
      </div>
  ...
  return (
    <section className="mt-6">
      <div className="rounded-2xl border border-neutral-200 bg-white p-3 md:p-4 shadow-sm">
              ...
              return (
                ...
                >
                  {isEditing ? (
                    ...
                    <button
                      className={cls("whitespace-nowrap px-1 text-sm", active ? "text-white" : "text-neutral-800 hover:text-neutral-900")}
                  ...
                  {!isEditing && (
                    <div className="flex items-center gap-1">
                      <IconButton
                        title="เปลี่ยนชื่อ"
                        onClick={() => { setEditingId(c.id); setDraft(label); }}
                        variant={active ? "ghost-white" : "ghost"}
                      >
                        {PencilIcon()}
                      </IconButton>
                      <IconButton
                      ...
                      <IconButton
                        title={onlyOne ? "เหลือได้อย่างน้อย 1 รายชื่อ" : `ลบ “${label}”`}
                        onClick={() => !onlyOne && onDelete(c.id, label)}
                        variant={active ? "ghost-white" : "ghost"}
                        disabled={onlyOne}
                      >
                        {TrashIcon()}
        ...
        <p className="mt-1 text-xs text-neutral-500">ทิป: ดับเบิลคลิกที่แท็บเพื่อแก้ชื่ออย่างรวดเร็ว • หากเหลือรายชื่อเดียว ให้ใช้ปุ่ม “รีเซ็ต (รายชื่อนี้)” เพื่อเริ่มใหม่</p>
      </div>
    </section>
  );
}
...
function IconButton({ children, onClick, title, variant = "ghost", disabled = false }) {
  ...
  const theme = variant === "ghost-white"
    ? "hover:bg-white/20 text-white"
    : "hover:bg-neutral-100 text-neutral-600";
  return (
    <button className={`${base} ${theme} ${disabled ? 'opacity-40 cursor-not-allowed' : ''}`} onClick={onClick} title={title} aria-disabled={disabled}>
      {children}
    </button>
  );
}
...
function labelOf(c) {
  return c.label || c.applicant?.fullName || "";
}
              ...
              <div className="flex flex-wrap items-center gap-3">
                <fieldset className="flex items-center gap-3">
                  ...
                  <label className="inline-flex items-center gap-2 text-sm">
                    <input type="radio" name="nationality" checked={profile.nationality === "Thai"} onChange={() => setProfile({ nationality: "Thai" })} /> Thai
                  </label>
                  <label className="inline-flex items-center gap-2 text-sm">
                    <input type="radio" name="nationality" checked={profile.nationality === "Non-Thai"} onChange={() => setProfile({ nationality: "Non-Thai" })} /> Non-Thai
                  </label>
                </fieldset>
                <label className="inline-flex items-center gap-2 text-sm">
                  <input type="checkbox" checked={!!profile.phd} onChange={(e) => setProfile({ phd: e.target.checked })} /> มีเอกสารสำหรับ PhD (แสดงหมวด PhD)
                </label>
              </div>
            </div>
...
function SectionCard({ section, onToggleOpen, onToggleItem, onUpdateItem, onRemoveItem, onAddItem }) {
  ...
  // Local state for add-item controls
          ...
          <div className="mt-3 flex flex-col gap-2 rounded-xl border border-dashed border-neutral-300 p-3">
            ...
            <div className="flex flex-col gap-2 md:flex-row">
              ...
              <button
                className="btn"
                onClick={() => {
                  onAddItem(section.id, draftLabel, draftReq);
                  setDraftLabel("");
                  setDraftReq("optional");
                }}
              >
                เพิ่มรายการ
              </button>
  ...
  return (
    <li
      className={cls(
        ...
        item.status === "done" ? "border-emerald-300 bg-emerald-50" : "border-neutral-200 bg-white"
      )}
    >
      <div className="flex flex-col gap-2 md:flex-row md:items-start md:gap-4">
        <div className="flex items-start gap-3 md:w-1/2">
          <input
            ...
            onChange={onToggle}
            className="mt-1 h-5 w-5 rounded border-neutral-300 text-indigo-600 focus:ring-indigo-500"
            ...
            <div className="flex flex-wrap items-center gap-2">
              ...
              <span
                className={cls(
                  ...
                  item.req === "required" ? "bg-rose-100 text-rose-800" : "bg-neutral-100 text-neutral-700"
                )}
              >
                {item.req === "required" ? "จำเป็น" : "ขึ้นกับเงื่อนไข"}
              </span>
              {item.custom && (
                <span className="rounded-full bg-indigo-100 text-indigo-800 px-2 py-0.5 text-[11px]">custom</span>
              )}
            </div>
        ...
        <div className="md:w-1/2 grid gap-2">
          <div className="flex gap-2">
            <input
              ...
              placeholder="ชื่อไฟล์/ลิงก์เอกสาร (ตัวอย่าง: transcript_en.pdf หรือ Google Drive URL)"
              className="flex-1 rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
              aria-label="ชื่อไฟล์หรือที่อยู่ลิงก์"
            />
          ...
          <textarea
            ...
            placeholder="บันทึกโน้ต เช่น แปลอังกฤษเรียบร้อย ตรารับรองครบ"
            className="min-h-[42px] w-full rounded-lg border border-neutral-300 bg-white px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-indigo-500"
    ...
    <div role="dialog" aria-modal="true" className="fixed inset-0 z-20 grid place-items-center bg-black/40 p-4" onClick={onClose}>
      <div className="w-full max-w-lg rounded-2xl bg-white p-5 shadow-xl" onClick={(e) => e.stopPropagation()}>
        ...
        <div className="mt-3 flex flex-wrap items-center gap-3">
          ...
          <label className="inline-flex items-center gap-2 text-sm">
            <input type="checkbox" checked={phd} onChange={(e) => setPhd(e.target.checked)} /> PhD
          </label>
        </div>
        <div className="mt-4 flex gap-2 justify-end">
          <button className="btn-secondary" onClick={onClose}>ยกเลิก</button>
          <button className="btn" onClick={() => onCreate({ fullName, degree, institution, country, nationality, phd })}>
            สร้างแบบฟอร์ม
          </button>
        </div>
      </div>
...
// -----------------------------------------------------------------------------
// Reusable button styles via Tailwind utility classes
const baseBtn = "inline-flex items-center justify-center rounded-xl px-3.5 py-2 text-sm font-medium transition-colors focus:outline-none focus:ring-2";
const btnStyles = `
  .btn { ${baseBtn}; background-color: rgb(79 70 229); color: white; }
  .btn:hover { background-color: rgb(67 56 202); }
  .btn-secondary { ${baseBtn}; background-color: white; color: rgb(17 24 39); border: 1px solid rgb(209 213 219); }
  .btn-secondary:hover { background-color: rgb(249 250 251); }
  .btn-danger { ${baseBtn}; background-color: rgb(244 63 94); color: white; }
  .btn-danger:hover { background-color: rgb(225 29 72); }
`;
...
// - Sync ชื่อแท็บกับชื่อ-นามสกุลโดยอัตโนมัติ (ถ้าช่อง label ว่างอยู่)
// - Duplicate แบบเลือก: เลือกหมวดที่จะคัดลอก (เช่น เฉพาะโปรไฟล์ ไม่เอาสถานะเช็คลิสต์)
// - Drag & reorder รายชื่อ (ลากสลับลำดับ)
// - Quick share: ปุ่มสร้างลิงก์ snapshot read-only
